...
function GetYieldTooltipHelper(pCity, iYieldType, strIcon)
	...
	if (iYieldType == YieldTypes.YIELD_GOLD) then
		...
		iBaseYield = iBaseYield + iYieldFromCorporations;
		iTotalYield = iTotalYield + (iYieldFromCorporations * (pCity:GetBaseYieldRateModifier(YieldTypes.YIELD_GOLD) / 100));				
	end
	-- CORPORATIONS End
...
function GetYieldTooltip(pCity, iYieldType, iBase, iTotal, strIconString, strModifiersString)
	...
	-- CORPORATIONS Start
	if (iYieldType == YieldTypes.YIELD_GOLD) then
		-- Base Yield from Corporations
		local iYieldFromCorporations = GetCorporationRevenueForCity(pCity);	
		if (iYieldFromCorporations ~= 0) then
			strYieldBreakdown = strYieldBreakdown .. "[ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_YIELD_FROM_CORP", iYieldFromCorporations, strIconString);
			strYieldBreakdown = strYieldBreakdown .. "[NEWLINE]";
		end
	end
	-- CORPORATIONS End
