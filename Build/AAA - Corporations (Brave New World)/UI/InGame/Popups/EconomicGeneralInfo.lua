...
function UpdateGPT()
	...
	-- CORPORATIONS Start
	local corpRevenue = GetCorporationRevenue(pPlayer);
	local grossGoldValue = pPlayer:CalculateGrossGoldTimes100() / 100;
	grossGoldValue = grossGoldValue + corpRevenue;
	Controls.GrossGoldValue:SetText( "[COLOR_POSITIVE_TEXT]" .. Locale.ToNumber( grossGoldValue, "#.##" ) .. "[ENDCOLOR]" );
	...
	-- CORPORATIONS Start
    local corpRevenue = GetCorporationRevenue(pPlayer);
    if( corpRevenue > 0 ) then
        Controls.CorporationIncomeValue:SetText( Locale.ToNumber( corpRevenue, "#.##" ) );
    else
        Controls.CorporationIncomeValue:SetText( 0 );
    end
	-- CORPORATIONS End
