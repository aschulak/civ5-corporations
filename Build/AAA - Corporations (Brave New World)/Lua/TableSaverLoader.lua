...
-- Author: Pazyryk
------------------------------------------------------------------------------------------------------
-- Version  0.10, 10/4/11 initial beta release
--			0.11,	minor fixes
------------------------------------------------------------------------------------------------------
-- Some easy rules:
...
function TableLoad(globalTable, DBTablePrefix)
	...
	while i do
		...
		if endlessLoopCounter > 100000 then
			print (DBTableData.." is corrupt: could not find parents for "..#loadBuffer.parent.." rows (e.g., "..loadBuffer.parent[i]..")")
			break
		end
	end
...
function DoDBTypeTextUpdates()
	...
	for i = 1, items do
		...
		print(id, idStr, dbType[id], dbText[id])
