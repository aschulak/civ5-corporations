		...
		if (pPlayer:GetNumCities() > 0) then
			...
			if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_SCIENCE)) then
				...
				-- We have science
					...
					-- CORPORATIONS Start
					local fCorpGold = GetCorporationRevenue(pPlayer);					
					--local iGoldPerTurn = pPlayer:CalculateGoldRate();
					local iGoldPerTurn = pPlayer:CalculateGoldRate() + fCorpGold;
					-- CORPORATIONS End
...
function GoldTipHandler( control )
	...
	if (math.floor(fCorpGold) > 0) then
		strText = strText .. "[NEWLINE]  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_GOLD_FROM_CORP", math.floor(fCorpGold));
	end	
	-- CORPORATIONS End
